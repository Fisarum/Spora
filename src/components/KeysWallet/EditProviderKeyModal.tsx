...
interface Props {
  ...
  onClose: () => void;
  onUpdated: (key: ProviderKey) => void;
}

export default function EditProviderKeyModal({ providerKey, onClose, onUpdated }: Props) {
  const [provider, setProvider] = useState<Provider>(providerKey.provider);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError(null);
    try {
      await keysApi.updateProviderKey({
        id: providerKey.id,
        provider,
        label: label.trim(),
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md">
      <div className="w-full max-w-md mx-4 rounded border border-primary/20 bg-background shadow-2xl">
        <div className="flex items-center justify-between p-5 border-b border-primary/10">
          <h3 className="text-sm tracking-[0.2em] text-primary uppercase font-medium">Edit Provider Key</h3>
          <button
            onClick={onClose}
