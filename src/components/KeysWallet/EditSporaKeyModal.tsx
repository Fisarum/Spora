...
interface Props {
  sporaKey: SporaKey;
  onClose: () => void;
  onUpdated: (key: SporaKey) => void;
}

export default function EditSporaKeyModal({ sporaKey, onClose, onUpdated }: Props) {
  const [label, setLabel] = useState(sporaKey.label);
  const [location, setLocation] = useState(sporaKey.location || "");
  const [restrictProviders, setRestrictProviders] = useState(!!sporaKey.allowedProviders);
  const [selectedProviders, setSelectedProviders] = useState<Provider[]>(sporaKey.allowedProviders || []);
  const [dailyLimit, setDailyLimit] = useState(sporaKey.dailyLimitUsd?.toString() || "");
  const [monthlyLimit, setMonthlyLimit] = useState(sporaKey.monthlyLimitUsd?.toString() || "");
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      onUpdated({
        ...
        label: label.trim(),
        location: location.trim(),
        allowedProviders: restrictProviders && selectedProviders.length > 0 ? selectedProviders : null,
        dailyLimitUsd: dailyLimit ? parseFloat(dailyLimit) : null,
        monthlyLimitUsd: monthlyLimit ? parseFloat(monthlyLimit) : null,
      });
    } catch (err: unknown) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md">
      <div className="w-full max-w-md mx-4 rounded border border-primary/20 bg-background shadow-2xl">
        <div className="flex items-center justify-between p-5 border-b border-primary/10">
          <h3 className="text-sm tracking-[0.2em] text-primary uppercase font-medium">Edit Spora Key</h3>
          <button
            onClick={onClose}
